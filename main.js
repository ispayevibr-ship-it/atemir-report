...
async function checkHotUpdate(){try{let raw=await ghGet("https://api.github.com/repos/"+HOT_OWNER+"/"+HOT_REPO+"/contents/hot/latest.json?ref=main"),meta=JSON.parse(Buffer.from(JSON.parse(raw).content||"","base64").toString("utf8"));if(!meta.version)return false;if(verCmp(meta.version,effectiveVersion())<=0){hotUpdate=null;return "current"}hotUpdate=meta;sendUpdate("available",{version:meta.version,sizeBytes:meta.size||0,light:true});return true}catch(e){console.error("Hot update check:",e.message);return false}}
...
ipcMain.on("app:update-check",async()=>{sendUpdate("checking");hotUpdate=null;let hot=await checkHotUpdate();if(hot==="current"){sendUpdate("none",{version:effectiveVersion()});return}if(!hot)autoUpdater.checkForUpdates().catch(e=>sendUpdate("error",{message:e.message}))});
