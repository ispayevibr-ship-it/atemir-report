...
ipcMain.handle("file:pdf",async()=>{const r=await dialog.showSaveDialog(win,{defaultPath:"Отчет-СМР.pdf",filters:[{name:"PDF",extensions:["pdf"]}]});if(r.canceled)return false;const b=await win.webContents.printToPDF({printBackground:true,pageSize:"A4"});fs.writeFileSync(r.filePath,b);return true});
