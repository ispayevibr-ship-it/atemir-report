...
async function checkHotUpdate(){try{let raw=await ghGet("https://api.github.com/repos/"+HOT_OWNER+"/"+HOT_REPO+"/contents/hot/latest.json?ref=main"),meta=JSON.parse(Buffer.from(JSON.parse(raw).content||"","base64").toString("utf8"));if(!meta.version||verCmp(meta.version,app.getVersion())<=0)return false;hotUpdate=meta;sendUpdate("available",{version:meta.version,sizeBytes:meta.size||0,light:true});return true}catch(e){console.error("Hot update check:",e.message);return false}}
