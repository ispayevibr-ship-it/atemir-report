...
let win,db;
function initDb(){const dir=app.getPath("userData");fs.mkdirSync(dir,{recursive:true});db=new Database(path.join(dir,"atemir.sqlite"));db.pragma("journal_mode=WAL");db.exec("CREATE TABLE IF NOT EXISTS state(id INTEGER PRIMARY KEY,json TEXT NOT NULL,updated TEXT NOT NULL);CREATE TABLE IF NOT EXISTS reports(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,json TEXT NOT NULL,updated TEXT NOT NULL)");
 const count=db.prepare("SELECT COUNT(*) n FROM reports").get().n;if(!count){const old=db.prepare("SELECT json FROM state WHERE id=1").get();if(old){let d={};try{d=JSON.parse(old.json)}catch(e){}const name=d.object||d.objectName||"Первый отчёт";db.prepare("INSERT INTO reports(name,json,updated) VALUES(?,?,datetime('now'))").run(name,old.json)}}}
