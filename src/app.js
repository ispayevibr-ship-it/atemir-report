...
async function openReport(id){await flushSave();const d=await ATemir.load(+id);if(!d)return;currentReportId=+id;state=normalizeState(d);hydrateRowUnits();financeUnlocked=false;view="home";render()}
...
$("#reportsBtn").onclick=async()=>{await flushSave();await refreshReports();view="reports";render()};
...
$("#imp").onclick=async()=>{try{const d=await ATemir.importJSON();if(d){state=normalizeState({...state,...d});hydrateRowUnits();await save(true);render();alert("SaveFiles импортирован и сохранён в SQLite.")}}catch(e){alert("Ошибка импорта: "+e.message)}};$("#bak").onclick=()=>ATemir.backup(state);$("#pdf").onclick=()=>ATemir.pdf(cleanReportHTML());$("#html").onclick=()=>ATemir.exportHTML(cleanReportHTML());
