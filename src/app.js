...
function financeContractTotal(c){const base=n(c.qty)*n(c.price),vat=n(c.vatRate);return {base,vatAmount:base*vat/100,total:base*(1+vat/100)}}
...
function financeView(){
 ...
 const contracts=card("Договоры",rowEditor("financeContracts",[["Договор","contract"],["Вид работ / предмет","work"],["Ед. изм.","unit"],["Количество по договору","qty","number"],["Цена за 1 ед.","price","number"],["НДС","vatRate","vat"]])+`<table><thead><tr><th>Договор</th><th>Без НДС</th><th>НДС</th><th>С НДС</th></tr></thead><tbody>${cs.map(c=>{const x=financeContractTotal(c);return `<tr><td>${esc(c.contract||"—")}</td><td>${money(x.base)}</td><td>${n(c.vatRate)?n(c.vatRate)+"% · "+money(x.vatAmount):"Без НДС"}</td><td><b>${money(x.total)}</b></td></tr>`}).join("")}</tbody></table>`);
 const actCard=card("Акты выполненных работ",rowEditor("financeActs",[["Дата акта","date","date"],["Договор","contract"],["№ акта","actNo"],["Закрытый объём","qty","number"],["Сумма акта с НДС","amount","number"]])+'<p class="hint">Если сумму акта не вводить, она рассчитывается по закрытому объёму × цене договора с учётом НДС.</p>');
