...
function cleanReportHTML(){const title=esc(state.object||"Отчёт"),company=esc(state.companyName||"");return `<!doctype html><html lang="ru"><meta charset="utf-8"><title>${title}</title><style>@page{size:A4;margin:10mm}*{box-sizing:border-box}body{font-family:Arial;margin:0;color:#183247}h1{font-size:20px;margin:8px 0 14px}h2{font-size:14px}.card{margin-bottom:15px;break-inside:avoid}.card h2{border-bottom:2px solid #f2b318}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #ccc;padding:6px}.photoCollage{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}.photoCollage img{width:100%;height:130px;object-fit:cover}.bars{height:180px;display:flex;align-items:flex-end;gap:4px}.barCol{height:100%;min-width:28px;display:flex;flex-direction:column;justify-content:flex-end;align-items:center}.barCol i{width:70%;background:#123e63}.barCol small,.barCol span{font-size:7px}.gTrack{height:18px;background:#dfe8f0;position:relative}.gFact{display:block;height:100%;background:#1769aa}.gToday{position:absolute;top:-4px;bottom:-4px;width:2px;background:#e0a800}.calendar{display:grid;grid-template-columns:repeat(7,1fr);gap:2px}.calDay{min-height:26px;border:1px solid #ddd}.calDay.work{background:#dff4e5}.calDay.acted{background:#fbe0e0}.multiCalendar{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.stats{display:flex;gap:8px;flex-wrap:wrap}.stats b{padding:5px 8px;border-left:3px solid #f2b318;background:#eef3f7}.bomFilters{display:none}.reportGroup{border:1px solid #ddd;margin:5px 0}.reportGroup summary{padding:6px}.reportGroup>table{display:table!important}.reportLogo img{max-width:210px;max-height:80px}.gantt{break-inside:avoid;margin:7px 0}.monthCard{break-inside:avoid}.empty{color:#777;font-size:10px}@media print{details>*{display:block!important}summary{list-style:none}}</style><body>${company?`<h2>${company}</h2>`:""}<h1>${title}</h1>${home()}</body></html>`}
