...
let state={object:"",address:"",client:"",reportDate:"",weather:{temp:"",wind:"",precip:""},workTypes:[],tasks:[],workDays:[],invoices:[],workers:[],responsibles:[],equipment:[],deadlines:[],actedDays:[],penalties:[],reportPhotos:[],companyLogo:""},view="home",manageView="object";
...
function render(){$("#nav").innerHTML=views.map(x=>`<button data-v="${x[0]}" class="${view===x[0]?"active":""}">${x[1]}</button>`).join("");$("#title").textContent=views.find(x=>x[0]===view)?.[1]||"Отчёт";$("#obj").textContent=state.object||state.objectName||"Новый объект";const c=$("#content");
 if(view==="home")c.innerHTML=home();else if(view==="progress")c.innerHTML=summary();else if(view==="works")c.innerHTML=card("Выполненные работы",table(arr("workDays"),[["Дата","date"],["Работа","type"],["Шифр","code"],["Марка","mark"],["Наименование","name"],["Количество",r=>r.qty||r.quantity],["Ед.","unit"]]));else if(view==="bom")c.innerHTML=bomView();else if(view==="invoices")c.innerHTML=card("Поставка",table(arr("invoices"),[["Дата","date"],["Работа","type"],["Шифр","code"],["Марка","mark"],["Количество",r=>r.qty||r.quantity]]));else if(view==="people")c.innerHTML=card("Ответственные / работники",table([...arr("responsibles"),...arr("workers")],[["ФИО",r=>r.name||r.fio],["Должность","position"]]));else if(view==="machines")c.innerHTML=card("Машины и механизмы",table(arr("equipment"),[["Наименование",r=>r.name||r.title],["Количество",r=>r.qty||r.quantity]]));else if(view==="dynamics")c.innerHTML=dynamicsView();else if(view==="deadlines")c.innerHTML=ganttView();else if(view==="acted")c.innerHTML=card("Актированные дни",table(arr("actedDays"),[["Дата",r=>r.date||r]]))+calendarView();else if(view==="penalties")c.innerHTML=card("Штрафы",table(arr("penalties"),[["Дата","date"],["Описание",r=>r.text||r.reason||r.name],["Сумма","amount"]]));else c.innerHTML=management();
}
$("#nav").onclick=e=>{const b=e.target.closest("[data-v]");if(b){view=b.dataset.v;render()}};
$("#content").onclick=async e=>{const pa=e.target.closest("[data-photo-add]");if(pa){const imgs=await ATemir.choosePhotos();if(imgs?.length){state.reportPhotos=[...arr("reportPhotos"),...imgs].slice(0,12);await save(true);render()}return}const pd=e.target.closest("[data-photo-del]");if(pd){arr("reportPhotos").splice(+pd.dataset.photoDel,1);await save(true);render();return}const pc=e.target.closest("[data-photo-clear]");if(pc){state.reportPhotos=[];await save(true);render();return}const ll=e.target.closest("[data-logo-load]");if(ll){const data=await ATemir.chooseLogo();if(data){state.companyLogo=data;await save(true);render()}return}const lr=e.target.closest("[data-logo-remove]");if(lr){state.companyLogo="";await save(true);render();return}const m=e.target.closest("[data-mv]");if(m){manageView=m.dataset.mv;render();return}const a=e.target.closest("[data-add]");if(a){state[a.dataset.add]=arr(a.dataset.add);state[a.dataset.add].push({});await save(true);render();return}const d=e.target.closest("[data-del]");if(d){arr(d.dataset.del).splice(+d.dataset.i,1);await save(true);render()}};
