...
function reportsScreen(){
 const rows=reportList.map(r=>`<div class="reportTile"><div><b>${esc(r.name||"Без названия")}</b><small>Изменён: ${esc(r.updated||"—")}</small></div><div><button class="btn" data-report-open="${r.id}">Открыть</button><button data-report-dup="${r.id}">Дублировать</button><button class="danger" data-report-del="${r.id}">Удалить</button></div></div>`).join("");
