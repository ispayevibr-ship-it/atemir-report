...
$("#imp").onclick=async()=>{try{const d=await ATemir.importJSON();if(d){state=normalizeState({...state,...d});await save(true);render();alert("SaveFiles импортирован и сохранён в SQLite.")}}catch(e){alert("Ошибка импорта: "+e.message)}};$("#bak").onclick=()=>ATemir.backup(state);$("#pdf").onclick=()=>ATemir.pdf(cleanReportHTML());$("#html").onclick=()=>ATemir.exportHTML(cleanReportHTML());
(async()=>{const d=await ATemir.load();state=normalizeState(d?{...state,...d}:state);render()})().catch(e=>alert("Ошибка запуска: "+e.message));
